  ...
  return (
    <div className="relative grid place-items-center" aria-hidden>
      ...
      {[0, 1, 2].map((i) => (
        <motion.div
          key={i}
          className="absolute rounded-full border border-[#D97757]/20"
      ...
      {/* core */}
      <motion.div
        className="glow-primary relative h-44 w-44 rounded-full bg-[radial-gradient(circle_at_35%_30%,rgba(242,200,143,0.95),rgba(217,119,87,0.55)_40%,rgba(192,101,69,0.4)_75%,transparent)]"
    ...
    >
      ...
      <div className="grid gap-2">
        {chain.map((row, i) => (
          ...
          >
            <span className="w-40 shrink-0 text-[11px] font-bold tracking-wider text-[#F2C88F]">{row.k}</span>
...
export function Hero() {
  ...
  return (
    <section id="learn" className="relative overflow-hidden pb-24 pt-32 sm:pt-40">
      {/* aurora blobs */}
      <div className="aurora-blob left-[8%] top-[12%] h-72 w-72 bg-[#D97757]/15" />
      <div className="aurora-blob right-[5%] top-[45%] h-80 w-80 bg-[#F2C88F]/10" />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mx-auto max-w-3xl text-center">
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="mx-auto mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs text-muted-foreground uppercase tracking-widest font-semibold"
          >
              ...
              return (
                ...
                >
                  <div
                    ...
                    className="cursor-default rounded-xl border border-white/10 bg-card/80 px-3 py-2 text-[11px] font-semibold tracking-widest text-foreground/80 backdrop-blur transition-all hover:border-[#D97757]/40 hover:text-[#F2C88F] focus-visible:border-[#D97757]/40 focus-visible:outline-none sm:text-xs"
                  >
                    {el.label}
                    <div className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 w-48 -translate-x-1/2 rounded-lg border border-[#D97757]/20 bg-popover/95 p-2.5 text-[11px] font-normal leading-snug normal-case tracking-normal text-popover-foreground opacity-0 shadow-xl transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
                      {el.insight}
                    </div>
                  </div>
                </motion.div>
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              {HERO_EXAMPLES.map((ex) => (
                <button
                  ...
                  className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:border-[#D97757]/30 hover:text-[#F2C88F]"
                >
                  {ex.text}
                </button>
              ))}
