...
function QuizCard({ quiz, index }: { quiz: Quiz; index: number }) {
  ...
  return (
    ...
    >
      <TiltCard className="rounded-3xl border border-[#E8E4DE] bg-white p-5 shadow-md shadow-black/[0.03]">
        ...
        <p className="mt-3 text-sm font-medium leading-relaxed text-[#2D2D2D]">{quiz.question}</p>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 rounded-xl border px-3.5 py-2.5 text-left text-sm transition-all ${
                  state === "idle" ? "border-[#E8E4DE] bg-white/60 hover:border-[#6C5CE7]/40 hover:bg-white"
                  : state === "right" ? "border-[#67C587]/40 bg-[#67C587]/[0.08] text-[#67C587]"
                  : state === "wrong" ? "border-rose-300/60 bg-rose-50 text-rose-600"
                  : "border-[#E8E4DE]/40 text-[#B0B0BA]"
                }`}
              >
                <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg border border-[#E8E4DE] bg-[#FFF8F0] text-[11px] font-bold text-[#5A5A5A]">
        ...
        <AnimatePresence>
          {answered && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden">
              <div className="mt-3 rounded-xl border border-[#6C5CE7]/15 bg-[#6C5CE7]/[0.04] p-3.5">
                <p className="text-xs font-bold uppercase tracking-widest text-[#6C5CE7]">Why</p>
                <p className="mt-1 text-sm leading-relaxed text-[#8A8A82]">{quiz.explanation}</p>
                <button
                  onClick={() => setSelected(null)}
                  className="mt-2 flex items-center gap-1 text-xs font-semibold text-[#6C5CE7] hover:underline"
                >
...
function Flashcard({ card, index }: { card: typeof FLASHCARDS[number]; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Front */}
        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-3xl border border-[#E8E4DE] bg-gradient-to-br from-white to-[#FFF8F0] p-6 shadow-md shadow-black/[0.03] [backface-visibility:hidden] hover-glow">
          <span className="text-4xl" role="img" aria-hidden>
            {CHARACTER_EMOJI[card.character] || card.character}
          </span>
          <p className="mt-4 text-2xl font-bold tracking-tight text-[#2D2D2D]">{card.term}</p>
          <p className="mt-2 text-xs text-[#B0B0BA]">Click to flip</p>
        </div>

        {/* Back */}
        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-3xl border border-[#6C5CE7]/20 bg-gradient-to-br from-[#6C5CE7]/5 to-white p-6 shadow-md shadow-black/[0.03] [backface-visibility:hidden] [transform:rotateY(180deg)] hover-glow">
          <p className="text-center text-sm leading-relaxed text-[#5A5A5A]">{card.definition}</p>
          <p className="mt-3 text-center text-xs text-[#8A8A82] italic">"{card.example}"</p>
          {showWhy && (
            <p className="mt-3 text-center text-xs leading-relaxed text-[#6C5CE7]">{card.whyItMatters}</p>
          )}
          {!showWhy && (
            <button
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mx-auto mb-12 max-w-2xl text-center">
          <div className="mb-4 flex justify-center">
            <Squiggle color="#4A7BF7" className="opacity-40" />
          </div>
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-[#4A7BF7]">
          ...
          {quizScore.total > 0 && (
            <p className="mt-3 text-sm text-[#8A8A82]">
