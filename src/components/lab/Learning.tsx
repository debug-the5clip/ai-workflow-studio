...
function QuizCard({ quiz, index }: { quiz: Quiz; index: number }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-sm font-medium leading-relaxed text-[#1C1C1C]">{quiz.question}</p>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 rounded-xl border px-3.5 py-2.5 text-left text-sm transition-all ${
                state === "idle" ? "border-[#E2E0DB] bg-white/60 hover:border-[#D97757]/40 hover:bg-white" :
                state === "right" ? "border-[#6B9E8A]/40 bg-[#6B9E8A]/[0.08] text-[#6B9E8A]" :
                state === "wrong" ? "border-rose-300/60 bg-rose-50 text-rose-600" :
                "border-[#E2E0DB]/40 text-[#9A968F]"
              }`}
            >
              <span className="grid h-6 w-6 shrink-0 place-items-center rounded-lg border border-[#E2E0DB] bg-[#F5F4F0] text-[11px] font-bold text-[#4A4A46]">
      ...
      <AnimatePresence>
        {answered && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="overflow-hidden">
            <div className="mt-3 rounded-xl border border-[#D97757]/20 bg-[#D97757]/[0.04] p-3.5">
              <p className="text-xs font-bold uppercase tracking-widest text-[#D97757]">Why</p>
              <p className="mt-1 text-sm leading-relaxed text-[#6B6B66]">{quiz.explanation}</p>
              <button
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-3xl border border-[#E2E0DB] bg-gradient-to-br from-white to-[#F5F4F0] p-6 shadow-md shadow-black/[0.04] [backface-visibility:hidden] hover-glow">
          ...
          <p className="mt-4 text-2xl font-bold tracking-tight text-[#1C1C1C]">{card.term}</p>
          <p className="mt-2 text-xs text-[#9A968F]">Click to flip</p>
        </div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center rounded-3xl border border-[#D97757]/30 bg-gradient-to-br from-[#D97757]/5 to-white p-6 shadow-md shadow-black/[0.04] [backface-visibility:hidden] [transform:rotateY(180deg)] hover-glow">
          ...
          {showWhy && (
            <p className="mt-3 text-center text-xs leading-relaxed text-[#D97757]">{card.whyItMatters}</p>
          )}
          {!showWhy && (
            <button
